...
class ModelRouter:
    ...
    def select(self, request: ModelRequest) -> tuple[ModelDescriptor, ...]:
        ...
        if selected_id is None:
            raise ModelNotFoundError("No model was requested and no default model is configured")
        candidates = (selected_id, *self._policy.fallback_model_ids)
        descriptors = tuple(self._registry.get_model(model_id) for model_id in dict.fromkeys(candidates))
        required = request.required_capabilities()
        compatible = tuple(model for model in descriptors if required <= model.capabilities)
        if not compatible:
            raise ModelCapabilityError(
                "No configured candidate supports the requested capabilities",
                safe_details={"required_capabilities": sorted(capability.value for capability in required)},
            )
        return compatible
